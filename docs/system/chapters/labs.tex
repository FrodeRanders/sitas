...
Each lab provides a command, observations, and a small modification. The aim is
to connect visible behavior to the mechanisms described earlier.
...
using a third-party runtime. Then read
\filepath{crates/sitas-core/src/executor/future.rs} and
\filepath{crates/sitas-core/src/executor/timer.rs}.
...
Timers do not run in their own thread. The executor records
...
Then read the task queueing code in
\filepath{crates/sitas-core/src/executor/task.rs} and
\filepath{crates/sitas-core/src/executor/task\_set.rs}.
...
A waker requests another poll; it does not flood the scheduler
with duplicate queue entries.

\section{Lab 3: Readiness-driven TCP}

\textit{Level: Intermediate.}
...
Readiness is retry-oriented. A writable notification does not mean
...
Scheduling groups are cooperative accounting classes, not
...
Snapshots are owned samples, not tracing events. To build a
...
The remote future is polled remotely, and the result crosses back
as an owned value.
...
Shard-local access is synchronous. To await, first produce an
...
CPU placement is an explicit request to the OS. It is not a load
balancer.

\section{Lab 9: Linux \texttt{io\_uring}}

\textit{Level: Advanced.}
...
Completion-based I/O has a different cancellation problem than
...
The runtime is most useful when a problem splits into owned
