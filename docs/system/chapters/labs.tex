...
something to observe, and a small modification to try. The goal is to connect
visible behavior to the mechanisms described earlier.

The labs are organized around ``why'' questions. Why does a timer need a waker?
Why does readiness require retrying? Why does cross-shard submission return an
owned result? Why does \texttt{io\_uring} need abandonment tracking? Running
the examples first makes those design choices concrete.

\begin{notabene}[Lab style]
Run one lab at a time. After each command, inspect the example source and the
runtime module it exercises. The examples are small enough that modifying them
is part of learning them.
\end{notabene}

\section{Lab 1: Timers and cooperative polling \hfill \textnormal{\textit{Beginner}}}
...
The lesson: timers do not run in their own thread. The executor records
deadlines, waits until the next event, and wakes tasks whose deadlines expire.

\section{Lab 2: Wake coalescing \hfill \textnormal{\textit{Beginner}}}
...
The lesson: a waker requests another poll; it does not flood the scheduler
with duplicate queue entries.

\section{Lab 3: Readiness-driven TCP \hfill \textnormal{\textit{Intermediate}}}
...
The lesson: readiness is retry-oriented. A writable notification does not mean
every byte has been written; the helper retries until the operation completes.

\section{Lab 4: Scheduling groups \hfill \textnormal{\textit{Intermediate}}}
...
TCP scheduling example shows which group live handler tasks belong to. The
local scheduling-group demo shows virtual runtime and charged poll time
changing over time.
...
The lesson: scheduling groups are cooperative accounting classes, not
preemptive priorities. Long-running futures must yield.

\section{Lab 5: Observability snapshots \hfill \textnormal{\textit{Beginner}}}
...
The lesson: snapshots are owned samples, not tracing events. To build a
console-like view, sample repeatedly and compute differences.

\section{Lab 6: Cross-shard submission \hfill \textnormal{\textit{Intermediate}}}
...
The lesson: the remote future is polled remotely, and the result crosses back
as an owned value.

\section{Lab 7: Shard-local state \hfill \textnormal{\textit{Intermediate}}}
...
The lesson: shard-local access is synchronous. To await, first produce an
owned value and await outside the local-state closure.

\section{Lab 8: CPU placement \hfill \textnormal{\textit{Intermediate}}}
...
The lesson: CPU placement is an explicit request to the OS. It is not a load
balancer.

\section{Lab 9: Linux \texttt{io\_uring} \hfill \textnormal{\textit{Advanced}}}
...
The lesson: completion-based I/O has a different cancellation problem than
readiness-based I/O. The kernel may still own an operation after the future is
gone.

\section{Lab 10: Sharded index build \hfill \textnormal{\textit{Advanced}}}
...
\begin{lstlisting}[language=bash,caption={Mailbox index build}]
cargo run --example sharded_index_mailbox -- --records 512 --shards 4 --seed 0xbeef
\end{lstlisting}

Observe the printed logical assembler placement. The default mailbox variant
uses one more assembler work unit than shards, so one shard owns two logical
receivers. Record keys route to assembler names first, and the placement table
then resolves those names to shards.
...
\begin{enumerate}
    ...
    \item Change \texttt{--assemblers} in the mailbox variant.
    \item Compare per-shard partition summaries and final verification output.
\end{enumerate}

The lesson: the runtime's value appears when a problem splits into owned
shard-local work combined through explicit owned results or messages.

\section{After the labs}

After running these labs, return to \autoref{chap:walkthroughs} and re-read the
modules. The source is easier to follow after seeing observable behavior. If a
result is surprising, use the debugging checklist
(\autoref{sec:debugging-checklist}) before changing code.
