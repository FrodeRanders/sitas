...
\begin{description}
    ...
    \item[Shard runtime] The portable \texttt{no\_std} contract for spawning
        shard threads, creating typed channels, parking, and constructing a
        reactor for each shard. Unix and CharlotteOS provide separate
        implementations.

    \item[Reactor backend] The portable interface through which a shard
        executor waits for events and receives cross-thread wakes.
    ...
    \item[Shard mailbox] A host-runtime bounded queue for transferring owned
        messages to one shard-local receiver. \texttt{ShardMailboxSet<M>} is
        the uniform form: one inbound mailbox per shard.

    \item[Portable shard channel] The \texttt{ShardRuntime} channel used by
        \texttt{no\_std} and foreign-runtime backends. Senders are cloneable,
        sends are serialized before entering the single-producer ring, and the
    ...
    \item[Executor join handle] An awaitable handle for a spawned task result.
        Aborting it requests task cancellation. Dropping it without awaiting
        also aborts the underlying task.

    \item[Shard thread join handle] A backend-provided blocking join for a
        shard thread. The Unix backend uses a thread join. CharlotteOS waits on
        a kernel exit-observer completion and returns the closure result.
...
\begin{description}
    \item[Running statistics] An online Welford accumulator for count, mean,
        variance, standard deviation, coefficient of variation, min, max, and
        total. Accumulators can be merged without retaining samples.
