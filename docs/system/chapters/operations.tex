...
Keeping the manual build simple is intentional. The documentation does not
depend on the runtime being runnable, Docker being available, or generated
diagrams. A plain text-to-PDF path makes it cheap to update design
explanations alongside code changes.
...
of the code, while Linux is the only honest target for Linux-specific
primitives like hard CPU affinity and \texttt{io\_uring}. The helper keeps that
platform check repeatable without pretending the project is platform-neutral
where it is not.
