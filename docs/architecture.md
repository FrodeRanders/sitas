...
The `sharded_index_build` example demonstrates this shape on a fixed-record file: each shard scans and sorts one data-file partition into a local index run, then merge rounds are submitted back onto shards before the final sorted offset index is written.
