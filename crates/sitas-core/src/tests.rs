...
fn block_on_preserves_root_future_panic() {
    let panic = core::panic::catch_unwind(|| {
        block_on(async {
            panic!("root panic");
        });
    })
...
fn io_uring_read_and_write_are_driven_by_executor_loop() -> io::Result<()> {
    ...
    let (bytes, snapshot) = executor.run_until(async {
...
impl<T> Future for CatchJoinPanic<T> {
    ...
    fn poll(
        ...
        let result = core::panic::catch_unwind(core::panic::AssertUnwindSafe(|| {
            core::pin::Pin::new(&mut self.handle).poll(context)
        }));

        match result {
            Ok(core::task::Poll::Ready(Err(error))) if error.is_panic() => {
                *self.observed.lock().unwrap() = true;
                core::task::Poll::Ready(())
            }
            Ok(core::task::Poll::Ready(_)) => core::task::Poll::Ready(()),
